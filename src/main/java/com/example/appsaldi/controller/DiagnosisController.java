...
public class DiagnosisController implements Initializable {
    ...
    @Setter private UsersModel usersModel;
    ...
    private final DiagnosaDAO diagnosaDAO = new DiagnosaDAO(connection);
    ...
    private void handleDiagnosisButton() {
        try {
            ...
            List<Integer> daftarIdPenyakit = diagnosaDAO.diagnosaPenyakit(daftarGejalaDipilih);
            ...
            for (Integer idPenyakit : daftarIdPenyakit) {
                String namaPenyakit = diagnosaDAO.getNamaPenyakitById(idPenyakit);
                String solusi = diagnosaDAO.getSolusiById(idPenyakit);
            ...
            List<Integer> idGejalaList = daftarGejalaDipilih.stream()
                    ...
                    .toList();
            controller.setDiagnosisData(selectedPasien.getIdPendaftaran(), selectedPasien.getIdPasien(), daftarIdPenyakit, idGejalaList, diagnosaDAO);
    ...
    private void showPopupDataPasien(){
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/appsaldi/view/popup/popupdatapendaftaran.fxml"));
            Parent root = loader.load();
            PopupDataPendaftarController controller = loader.getController();
            controller.setListener(this::setDataPasein);
            Stage mainStage = (Stage) btnCari.getScene().getWindow();
            Stage popupStage = new Stage();
            popupStage.initModality(Modality.WINDOW_MODAL);
            popupStage.initOwner(mainStage);
